...
team_counts_all <- pbp_data_team %>%
  filter(game_seconds_remaining >= 300, play_type %in% c("run", "pass", "field_goal", "punt"), defteam == team) %>%
  mutate(play_category = case_when(
    play_type == "run" & qb_scramble == 0 ~ 0L,
    play_type == "pass" | qb_scramble == 1 ~ 1L,
    play_type == "field_goal"             ~ 2L,
...
output_csv <- paste0("team-data/", team, "/coach_decision_probs_def_", team, ".csv")
