...
void loadPriorData(const string& filename, vector<double>& data) {
    ...
    while (getline(file, line)) {
        ...
        // Store max_ep only if down == 1 and yardline matches criteria
...
vector<string> generateFilenames(string directory) {
    vector<string> filenames;
    for (const auto& play_type : play_types) {
        for (const auto& bin : yardline_bins) {
            filenames.push_back(directory+"/" + play_type + "_cdf_yl" + bin + ".json");
        }
    }
    return filenames;
}
...
void saveDataToCSV(string filename, unordered_map<pair<string, int>, double, pair_hash>& run_data,
    ...
    // Write CSV Header
    file << "Down,Distance,Yardline,Run_EP,Pass_EP,Kick_EP,Punt_EP,Max_EP,Opt_Choice\n";
...
// Function to read CSV into unordered_map
void loadPriorDataFromCSV(string filename,
        unordered_map<pair<string, int>, double, pair_hash>& run_data,
        unordered_map<pair<string, int>, double, pair_hash>& pass_data,
        unordered_map<pair<string, int>, double, pair_hash>& max_data) {
    ifstream file(filename);
    if (!file) {
    ...
    while (getline(file, line)) {
        ...
        string down_distance = to_string(down) + "-" + to_string(distance);
        pair<string, int> key = {down_distance, yardline};

        run_data[key] = run_ep;
        pass_data[key] = pass_ep;
        max_data[key] = max_ep;
    }

    file.close();
    cout << "CSV file loaded successfully: " << filename << endl;
}
...
unordered_map<pair<string, int>, double, pair_hash> prior_run_epas;
unordered_map<pair<string, int>, double, pair_hash> prior_pass_epas;
unordered_map<pair<string, int>, double, pair_hash> prior_max_epas;

vector<unordered_map<string, vector<int>>> nfl_sample_datas;

// Function to compute EPA values based on sampled results
double get_epa_val(int val, int down, int yards_to_go, int yardline) {

    if (val == -1e6) {
        return prior_epas[yardline-1];
    }
    ...
    auto key = make_pair(new_down_and_distance, new_yardline);
    if (max_epas.find(key) == max_epas.end()) {
        return prior_max_epas[key];  // Fallback to prior if available
    }
    return max_epas[key];
}
...
// Run the simulation
void run_simulation(vector<unordered_map<string, CDFEntry>>& cdf_datas, mt19937& mt1, vector<int>& yardline_mapping) {
    ...
    for (yardline = 1; yardline < 100; yardline++) {
        for (down = 4; down > 0; down--) {
            for (yards_to_go = 1; yards_to_go <= 20; yards_to_go++) {
                ...
                double epa_punt_val = get_epa_punt_val(yardline, mt1);   // only viable if it is 4th down
                ...
                if(!(cdf_datas[sample_num][down_and_distance].values.empty())){
                    ...
                    epa_rush_val = prior_run_epas[ddy];
                }
                if(!(cdf_datas[yardline_bins.size()+sample_num][down_and_distance].values.empty())){
                    ...
                    epa_pass_val = prior_pass_epas[ddy];
                }
...
int main(int argc, char* argv[]) {
    vector<string> filenames = generateFilenames("cdf_data");  // Generate filenames dynamically
    vector<unordered_map<string, CDFEntry>> cdf_datas(filenames.size());
    vector<string> nfl_filenames = generateFilenames("nfl_sample_data");
    nfl_sample_datas.resize(filenames.size());

    if(argc != 4){
        cout << "Need to input prior and target epa directories and a random seed: (./simulator.out prior_epas target_epas 20)" << endl;
        return 1;
    }

    string prior_data = argv[1];
    string target_data = argv[2];

    int seed_value = (argc >= 2) ? stoi(argv[3]) : SEED_VALUE;
    ...
    loadPriorData(prior_data+"/eps.csv", prior_epas);
    loadPriorDataFromCSV(prior_data+"/run_eps.csv", prior_run_epas, prior_pass_epas, prior_max_epas);
    ...
    run_simulation(cdf_datas, mt1, yardline_mapping);
    saveDataToCSV(target_data+"/eps.csv", run_epas, pass_epas, kick_epas, punt_epas, max_epas, opt_choices);
